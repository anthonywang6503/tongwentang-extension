...
| 併發請求數           |                                 `3` | 全部網頁共用的遠端請求上限，可設 `1–10`。 |

建議先使用預設值。連續失敗時會顯示一次低干擾通知，並在冷卻時間內直接走本機轉換；尚未送出的排隊請求也會改走本機轉換。冷卻結束後才會再次嘗試網路服務。

設定頁會即時顯示最近 300 筆 zhconvert 連線日誌，包含時間、耗時、轉換字數與連線失敗原因。可開啟「僅顯示失敗連線」以聚焦實際遠端連線失敗；此篩選只在目前設定頁有效。日誌只存在於背景程序記憶體，可手動清除，重新載入擴充功能後不會保留。
